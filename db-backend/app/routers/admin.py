...
from app.schemas.space import PricingPolicyCreateRequest, PricingPolicyUpdateRequest
...
def adjust_credit(user_id: int, body: CreditAdjustRequest, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    try:
        txn_type = "MANUAL_RESTORE" if body.changeScore > 0 else "ADJUST"
        user = credit_service.update_credit_score(
            db, user_id, body.changeScore, txn_type,
            operator_user_id=admin.user_id, remark=body.reason,
        )
        db.commit()
...
def create_policy(body: PricingPolicyCreateRequest, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    try:
        policy = pricing_service.create_policy(db, body.model_dump())
        db.commit()
        db.refresh(policy)
...
def update_policy(policy_id: int, body: PricingPolicyUpdateRequest, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    try:
        policy = pricing_service.update_policy(db, policy_id, body.model_dump(exclude_unset=True))
        db.commit()
        db.refresh(policy)
        return success(_policy_to_dict(policy))
    except ValueError as e:
...
def delete_policy(policy_id: int, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    try:
        policy = pricing_service.delete_policy(db, policy_id, admin.user_id)
        db.commit()
        return success(_policy_to_dict(policy))
    except ValueError as e:
        return fail(400, str(e))
